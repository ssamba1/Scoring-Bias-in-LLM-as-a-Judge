...
\]
An additional 22 instruct-only models provide model diversity breadth (29,700 additional (22 instruct-tuned models) judgments). Five models from the original OpenRouter list were excluded due to stop-token truncation producing uniform scores across all variants (Gemini-2.5-Pro, Inflection-3, Nemotron-Nano-30B, Command-R7B, Mistral-Small-24B).

\subsection{Inference}
For local models, we use greedy decoding (temperature 0). Kaggle T4 instances use fp16 with 4-bit quantization where available; P100 instances (PyTorch 2.13 dropped sm_60 support) fall back to CPU inference. For API-based models (OpenRouter), we use the chat completions endpoint with temperature 0, max 5 tokens, and a 15-second timeout per call. Total API cost under \$3.

\subsection{Experimental Setup}
Experiments were conducted on local CPU (analysis), Kaggle T4/P100 GPU with CPU fallback (primary model inference), and OpenRouter API (supplementary models). All random operations use seed 42. Key software: Python 3.11, PyTorch 2.0+, Transformers 4.47+, OpenAI Python SDK for API calls. Greedy decoding (temperature = 0) ensures deterministic generation for local models.
...
Having established the overall landscape, we now examine how bias varies by item domain, model characteristics, and individual model behavior.

\subsection{Domain Analysis}

Scoring bias varies across item domains. Table~\ref{tab:domain} shows bias averaged across 3 base-instruct families from the original Kaggle root cause study, broken down by 5 domains (10 items each). Humanities shows the highest bias in both base and instruct models, while Daily Life shows the lowest. Importantly, the bias reduction from base to instruct holds across all domains, confirming that instruction tuning's effect is not domain-specific.

\begin{table}[h]
\centering\small
\caption{Bias by item domain (10 items each) averaged across 3 base-instruct families.}
\label{tab:domain}
\begin{tabular}{lcc}
\toprule
...
Figure~\ref{fig:domain_bias} visualizes this domain breakdown. The differential effect holds across all five domains, demonstrating robustness to content variation.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/fig6_domain_bias.png}
\caption{Bias by item domain averaged across 3 base-instruct families. Blue: base models. Orange: instruct models. The differential effect holds across all 5 domains.}
\label{fig:domain_bias}
\end{figure}

\subsection{Error Analysis}

Figure~\ref{fig:error_analysis} examines the 5 least and 5 most biased models. The least biased models share low scores across all three probes; the most biased show susceptibility across multiple probes simultaneously.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/fig11_error_analysis.png}
\caption{Error analysis: least and most biased models with per-probe breakdown. Outliers (red) exceed $1.5\times$ IQR above Q3.}
\label{fig:error_analysis}
\end{figure}

\subsection{Item-Level Analysis}

Figure~\ref{fig:item_analysis} examines item difficulty vs discrimination across all probe variants for the 22 instruct models. Items with |r| > 0.3 are discriminating. The lack of strong positive discrimination indicates that bias is not driven by specific difficult items but is a general scoring property.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/fig9_item_analysis.png}
\caption{Item discrimination vs difficulty across 22 models. Top-5 discriminating items labeled. Lack of strong positive discrimination suggests bias is a general scoring property, not item-driven.}
\label{fig:item_analysis}
\end{figure}

Figure~\ref{fig:item_discrimination} provides an additional lens on item-level behavior, showing the distribution of discrimination values across all 50 items. Most items cluster near zero, confirming that no single item drives the observed bias.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/fig17_item_discrimination.png}
\caption{Item discrimination distribution across 50 items. Most items show near-zero discrimination, confirming bias is not item-driven.}
\label{fig:item_discrimination}
\end{figure}
