...
| `paper_biasinteraction.md` | A full draft of the bias-interaction paper, with a Results section | **Its stated results are the inverse of that project's measured ones.** The draft says position and verbosity "interact significantly" across all five judges, that "Claude and Llama show the strongest compounding (IR > 2.0)", and that "Gemini shows near-additive behavior". The completed study finds the interaction is entirely attributable to one judge — Gemini 2.5 Flash — with an equivalence test certifying additivity outright for the other four, and interactions accounting for about 1% of presentation-induced score variance. The named judges are swapped and a rejected universal claim is asserted. Nothing here was measured; it reads as a draft written before the data existed. `.hermes.md` named this file as the repository's main paper until 2026-08-13. |
