...
\author{Author Name$^{1}$, Author Name$^{1}$ \\
$^{1}$Department of Computer Science, School/Institution \\
...
% Import figures from paper/figures/study1/
% Compile with: cp paper/figures/study1/fig2_bias_by_probe.html figures/
% Or convert to PNG: screenshot your browser and save as figures/fig2.png
%\begin{figure}[h]
%\centering
%\includegraphics[width=\columnwidth]{figures/fig2_bias_by_probe.png}
%\caption{Mean bias ($\Delta$) by probe type for all 6 model variants. Error bars show 95\% bootstrap CIs.}
%\label{fig:bias}
%\end{figure}
...
We present the first systematic investigation of the origins of scoring bias in LLM-as-a-Judge. Our key finding---that instruction tuning has differential effects on scoring bias (format biases decrease, content biases increase)---provides the first experimental evidence that scoring bias is modulated by instruction tuning, not inherent to pre-training. This answers the open question posed by Li et al.~\cite{li2025scoring} and has direct implications for bias mitigation. All code, data, and analysis are publicly available at \url{https://github.com/ssamba1/research-draft}.
