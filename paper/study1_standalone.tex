...
We present the first systematic investigation of the origins of scoring bias in LLM-as-a-Judge. Our key finding---that instruction tuning has differential effects on scoring bias (format biases decrease, content biases increase)---provides the first causal evidence that scoring bias is modulated by instruction tuning, not inherent to pre-training. This answers the open question posed by Li et al.~\cite{li2025scoring} and has direct implications for bias mitigation. All code, data, and analysis are available at \url{https://github.com/ssamba1/research-draft}.
