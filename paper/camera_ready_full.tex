...
% ── ABSTRACT ──
\begin{abstract}
\textbf{Background:} LLMs deployed as automated judges exhibit systematic scoring biases, but whether these originate from pre-training or instruction tuning is unknown. \textbf{Methods:} We compare base and instruct variants across 3 model families and 22 additional models using 3 scoring bias probes. Format-related biases decrease by 44--77\%, but content-related bias increases by 35\%. The pattern holds across all RLHF families. Four alternative explanations are ruled out. \textbf{Conclusions:} Scoring bias is modulated by instruction tuning, not inherent to pre-training. Mitigation must address format and content channels separately.
\end{abstract}
