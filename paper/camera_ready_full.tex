...
\begin{enumerate}
    \item \textbf{Differential effect discovered:} Instruction tuning decreases format biases but increases content bias. The pattern is consistent across RLHF-trained families.
    \item \textbf{Alternative explanations ruled out:} We test and reject four competing hypotheses using the available data.
    \item \textbf{IIAR hypothesis proposed:} The Instruction-Induced Attention Redistribution framework explains the observed pattern with five testable predictions.
    \item \textbf{Open infrastructure:} Complete reproducible pipeline at \$0 cost, including code, data, and interactive analysis tools.
\end{enumerate}
...
\midrule
\textbf{This work} & \textbf{---} & \textbf{3(+22)} & \textbf{Yes} & \textbf{Yes} \\
\bottomrule
\end{tabular}
\end{table*}
