...
largest audit to date is \citet{norman2026reliability}: 21 judges from nine
...
(\S\ref{sec:anatomy}--\ref{sec:predict}).
...
attribution causal. We take this as convergent rather than competing, and it is a
sharper test of our account than any we ran ourselves: a stage that raises bias by
a third while adding no new nuisance sensitivity is what a decisiveness-driven
increase looks like, and the decomposition of \S\ref{sec:theory} predicts that
where a description of the same curve does not. Our contribution is therefore not
...
(\S\ref{sec:patch}--\ref{sec:spanpatch}), showing which representational change
...
here and flag as the most informative test either framework could run.
