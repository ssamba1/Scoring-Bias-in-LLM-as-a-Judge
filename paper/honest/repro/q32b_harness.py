# Scoring-bias harness (32B, NF4 4-bit; may not fit P100 -- fail-fast gamble) -- scale extension (P12) — logit scoring, per-item + argmax + domain.
...
if os.environ.get("R") != "1" and _cuda_bad():
    ...
    subprocess.run([sys.executable, "-m", "pip", "install", "-q", "transformers==4.49.0",
                    "tokenizers==0.21.0", "accelerate==1.4.0", "bitsandbytes==0.45.3"], check=False)
    os.environ["R"] = "1"
...
OUT_PATH = "/kaggle/working/results_32b.json"
...
PAIRS = [
    ("Qwen2.5-32B", "Qwen/Qwen2.5-32B", "Qwen/Qwen2.5-32B-Instruct", 32.0, "RLHF"),
]
if SMOKE:
    PAIRS = PAIRS[:2]
