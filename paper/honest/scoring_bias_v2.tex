...
confidence cannot. The findings replicate on public-dataset items and in Chinese; judge-side
\emph{sycophancy} shows the largest tuning effect we measure while numeric
\emph{anchoring} stays null; and the law survives its sharpest test: three deployed
frontier judges (GPT-4o among them), probed via API logprobs, are the most confident
\emph{and} most biased judges in the study, strengthening the pooled confidence--bias
...
\texttt{results\_probes2.json}, \texttt{results\_closed.json}; collected by
...
frontier-judge run (\S\ref{sec:frontier}) used the OpenRouter API: $\sim$4{,}500
single-token logprob calls, under US\$2 total. The environment is
