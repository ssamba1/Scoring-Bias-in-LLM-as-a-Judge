...
\emph{increases} bias across four of five bias types---so, across our \NFAM{} families,
more decisive judges are \emph{more} biased, not less (a negative decisiveness--bias
correlation). The explanation is a small theory: a judge's score is the mean of its
...
\begin{enumerate}\itemsep2pt
  \item \textbf{A counterintuitive empirical finding.} Instruction tuning makes small
        LLM judges sharper yet more biased across \NFAM{} families and five bias types;
        decisiveness correlates \emph{negatively} with bias, and bias is not predictable
        from decisiveness alone.
...
$H(\sigma)$ predicts bias out-of-sample (it does not---confirming a second factor is
required). A refuted prediction is the point: the theory is a decomposition of bias,
not a promise that decisiveness wins.
...
\MECHPROSE{}
\begin{figure}[t]\centering
\includegraphics[width=0.66\linewidth]{fig_mech.pdf}
\caption{(a) Instruction tuning lowers score-distribution entropy---judges become more
decisive (P1 holds). (b) Yet across all \NFAM{} families, five bias types, and both
checkpoints ($n=90$), entropy correlates \emph{negatively} with bias
($\rho=-0.51$)---decisive judges are more biased, refuting the naive reading of
Proposition~\ref{prop:main} and implicating the responsiveness term
(Corollary~\ref{cor:diff}).}
...
\subsection{Can bias be predicted from decisiveness alone? No.}\label{sec:predict}
If decisiveness alone governed bias, a judge's bias would be estimable from \emph{one
unperturbed forward pass}---its control-condition score entropy. We test this
out-of-sample with leave-one-family-out cross-validation. \PREDPROSE{}
\begin{figure}[t]\centering
\includegraphics[width=0.5\linewidth]{fig_predictor.pdf}
\caption{Predicting bias out-of-sample from control-condition entropy alone fails
(leave-one-family-out $R^2<0$): decisiveness is one factor of bias, not the whole of it.
