...
\paragraph{Confidence is the wrong screen.} A tempting shortcut would be to audit a
judge's bias from a single unperturbed call by reading its score entropy. Our predictor
experiment (\S\ref{sec:predict}) shows this fails ($R^2<0$): entropy alone does not
predict bias, precisely because the responsiveness term is unobserved in a single
unperturbed pass. Screening judges by confidence is therefore not only insufficient but
misleading---the most confident judges were often the most biased. \GOLDSENTENCE{}
\DISCUSSION{}
...
responsiveness rise dominates and bias increases across four of five types; the effective
