...
open-weight families (0.1--\MAXB{}B; 26 checkpoints; 13{,}000 per-item scores) and five
...
responsiveness, 84--99\% of the total rise; preference tuning installs the
confidence), a ground-truth test shows the bias corrupts real judgments
(\GOLDSENTENCE{}), and a marginalization mitigation cuts bias by 59\% where raising
confidence cannot. We also
expose a measurement confound---naive parse-based scoring silently drops all items on
weak judges---and remove it with the standard distribution-based readout. Every number and figure regenerates from committed raw files by committed scripts,
...
distribution (entropy $\times 0.71$) but inflates responsiveness ($\times 1.73$), and
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Mechanism (measured + causal).} Both terms of the decomposition are
        ...
        product predicts the per-cell direction of bias change under tuning (75\% sign
        ...
        alignment-stage ablation shows SFT installs the responsiveness (84--99\% of the
...
and make the distribution itself the object of analysis.
...
linear mixed-effects model with random intercepts for family and item. Because pooled
...
family-clustered bootstrap for the out-of-sample predictor.
...
\caption{Alignment-stage ablation (preregistered P7--P9). \textbf{(a)} Every stage
sharpens the score distribution, preference stages most. \textbf{(b)} Responsiveness
rises almost entirely at the \emph{SFT} step (84--99\% of the total rise).
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Verbosity's terse variant.} The padded variant adds content-free
        ...
        sentence, which can remove information. The verbosity $\Delta$ is therefore an
        upper bound on pure length bias; the padded-vs-control contrast alone, which is
        quality-preserving, shows the same direction (mean $|\Delta|$
        $0.29\!\to\!0.53$ after tuning, 11/13 families).
...
\texttt{repro/analyze\_mechanism.py}, \texttt{repro/analyze\_gold.py},
\texttt{repro/analyze\_robustness.py}, \texttt{repro/make\_mech\_figures.py}, and
\texttt{repro/make\_concept\_figure.py} from the committed raw files
(\texttt{results\_scaled.json}, \texttt{gold\_results.json}, \texttt{patch\_results.json},
\texttt{results\_multitemplate.json}, \texttt{attn\_results.json},
\texttt{results\_dolly.json.gz}, \texttt{results\_stages.json.gz}; collected by
\texttt{repro/\{scaled,gold,patch,multitemplate,attention,dolly,stage\}\_harness.py}), seed 42.
A continuous-integration check reruns every analysis script on the committed raw files
and fails on any numerical drift. No synthetic or
...
$|\kappa_3|$ from $1.03$ to $0.56$, and the excess kurtosis $\kappa_4$ from $-3.79$
