...
marginalization mitigation cuts bias by 59\% where raising confidence cannot. We also
expose a measurement confound---naive parse-based scoring silently drops all items on
weak judges---and remove it with the standard distribution-based readout. Every number and figure regenerates from one raw file by one
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Mechanism (measured + causal).} Both terms of the decomposition are
        ...
        causally carries the change in a localized network band.
...
\texttt{results\_multitemplate.json}, \texttt{attn\_results.json}; collected by
\texttt{repro/\{scaled,gold,patch,multitemplate,attention\}\_harness.py}), seed 42. No synthetic or
