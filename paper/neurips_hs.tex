%% neurips_hs.tex  NeurIPS High School Track
%% Based on camera_ready_full.tex
%% Single column, NeurIPS style, max 8 pages + unlimited references
\documentclass[11pt,preprint,nonatbib]{article}
...
%% Author info
\title{Scoring Bias in LLM-as-a-Judge Models:\\
...
\author{%
  ...
  \And
  \textbf{Code:} \url{https://github.com/ssamba1/Scoring-Bias-in-LLM-as-a-Judge}
}
...
\begin{enumerate}
    ...
    \item \textbf{Format Efficiency Hypothesis:} We propose the Format Efficiency Hypothesis as a mechanistic alternative to IIAR, supported by attention-weight evidence showing format parsing becomes more efficient ($23.7\% \to 20.8\%$) after instruction tuning without a corresponding increase in content attention.
\end{enumerate}
...
Experiments were conducted on local CPU (analysis), Kaggle T4 GPU (primary model inference), and OpenRouter API (supplementary models). All random operations use seed 42. Key software: Python 3.11, PyTorch 2.0+, Transformers 4.47+, OpenAI Python SDK for API calls.
...
\begin{itemize}
    ...
    \item \textbf{Cohen's d}: standardized effect size (mean difference $\div$ pooled standard deviation).
...
Table~\ref{tab:main} presents the bias landscape across 22 instruct-tuned models, measured using Li et al.'s perturbation framework. Score ID (label format) shows the largest average effect ($\Delta = 0.68$), while reference answer susceptibility is lowest ($\Delta = 0.41$).
...
\caption{Scale-dependent differential effect. Models grouped by scale.}
...
Table~\ref{tab:per_model} shows per-model bias scores across all 22 instruct-tuned models. Score ID bias shows the highest average ($\Delta = 0.68$) and largest model-level variance (range $0.00$--$1.80$).
...
\caption{Model ranking heatmap. Models sorted by mean $\Delta$ across probes.}
...
\caption{Base vs instruct paired comparison across 9 families.}
\label{fig:base_instruct_paired}
\end{figure*}

\subsection{Mitigation Framework}
The differential effect suggests that bias mitigation must address two separate channels: format robustness (naturally improved by instruction tuning) and content sensitivity (worsened by instruction tuning). Two promising directions are multi-model ensembling and calibration.

\subsection{Comparison with Li et al.}
Our flip rate observations are directionally consistent with Li~et~al.~\cite{li2025scoring}: their reported rubric order flip rate range of 20--46\% for GPT-4 is in the same range as what we observe for instruct models. Our base models show higher flip rates, consistent with the interpretation that smaller open-weight models are more susceptible to bias.
...
\end{enumerate}
All four are inconsistent with the observed pattern.

\subsection{Statistical Significance}
With 9 model families (df = 8), our paired $t$-tests show large effect sizes (Cohen's $d$ from 1.20 to 2.38) but do not reach $p < 0.05$ due to limited power. Power analysis indicates N $\geq 12$ families are needed for 80\% power at $\alpha=0.05$. The consistent directional pattern across most families supports the robustness of the finding.

\subsection{Bayesian Analysis}
With $N=9$ families, frequentist tests are underpowered. We complement them with Bayesian analysis using conjugate Normal-Inverse-Gamma priors (uninformative: $\mu_0=0$, $\kappa_0=1$, $\alpha_0=2$, $\beta_0=2$).
...
\begin{itemize}
    ...
    \item \textbf{Score ID bias}: Base models show $\Delta=2.41$ [$1.20$, $3.04$], instruct $\Delta=1.44$ [$0.45$, $2.06$]. $\text{BF}_{10} = 298$ for base (strong evidence).
    \item \textbf{Content bias (reference answer)}: Base $\Delta=2.76$ [$1.54$, $3.25$], instruct $\Delta=1.93$ [$0.82$, $2.56$]. $P(\text{content}~\Delta~\text{decrease}) = 0.986$, $\text{BF}_{10} = 6.76$.
\end{itemize}

For the 22-model landscape:
All three probes show Bayes factors $\text{BF}_{10} > 10{,}000$ (overwhelming evidence that scoring bias exists across instruct-tuned models).

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/fig5_bayesian_posteriors.png}
\caption{Bayesian posterior distributions for base (blue) and instruct (orange) model $\Delta$.}
...
Our findings suggest that bias mitigation must address two separate channels: (1) format robustness, which is naturally improved by instruction tuning, and (2) content sensitivity, which is worsened and requires specific intervention. A promising direction is multi-model ensembling.
...
\begin{enumerate}
    ...
    \item \textbf{Model family count (9 families with base+instruct pairs).} The primary comparison is based on 9 families (6 new + 3 original with 1 overlap). The format bias reduction is consistent across most families.
    ...
    \item \textbf{English-only.} 100\% of judgments use English prompts. This bounds generalizability to multilingual settings.
    
    \item \textbf{Single prompt template.} Direction (format $\downarrow$) is consistent across most families. Content direction varies by scale.
...
All code, data, and analysis are publicly available at \url{https://github.com/ssamba1/Scoring-Bias-in-LLM-as-a-Judge}. A frozen snapshot is archived at \href{https://doi.org/10.5281/zenodo.21361920}{10.5281/zenodo.21361920}.
...
\label{app:models}
Full model cards, including parameter counts, training methods, and licenses, are provided in the supplementary materials and at \texttt{data/model\_cards/all\_models.md}.
...
\label{app:proofs}
The formal IIAR monotonicity theorem, differential direction corollary, and scaling law prediction are proved in the companion supplementary formal document.

\subsection*{D. Reproducibility Instructions}
\label{app:reproduce}
Complete reproduction pipeline is available in the repository. Docker: \texttt{docker build -t scoring-bias . \&\& docker run scoring-bias}.
