...
LLMs serving as automated judges exhibit systematic scoring biases that undermine the reliability of AI evaluation. While prior work has identified these biases, the question of their origin—whether from pre-training or instruction tuning—has remained unanswered. We present a large-scale visual analysis of scoring bias across 31 model variants (54,000 judgments) comparing base and instruct-tuned models.
...
2. **Format-Content Scatter** (Fig. 2): 7/9 model families show a clear pattern—format bias decreases while content bias changes variably by scale. Models in the differential-effect quadrant cluster separately.
...
5. **Attention Evidence** (Fig. 8): Format token attention drops from 23.7% to 20.8% after instruction tuning—supporting the Format Efficiency Hypothesis.
