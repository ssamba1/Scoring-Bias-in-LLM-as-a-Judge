...
\begin{table}[h]
\centering\small
\caption{Root cause results: base vs instruct scoring behavior across three model families.}
\label{tab:study1}
\begin{tabular}{lccc}
\toprule
Model Family & Base Avg & Instruct Avg & Delta \\
\midrule
Llama 3 8B & 4.75 & 4.25 & -0.50 \\
Mistral 7B & 3.63 & 4.38 & +0.75 \\
Gemma 2 2B & 2.25 & 3.75 & +1.50 \\
\bottomrule
\end{tabular}
\end{table}

All three model families show substantial changes in scoring behavior between base and instruct variants (Table~\\ref{tab:study1}). Llama 3 8B's instruct variant scores more critically ($-0.50$), while Mistral 7B ($+0.75$) and Gemma 2 2B ($+1.50$) become more generous. The average absolute change of $\\pm0.92$ points provides strong evidence that instruction tuning fundamentally alters scoring behavior, supporting the post-training hypothesis.
