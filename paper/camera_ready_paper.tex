...
Score ID & 1.81 & 0.76 & -58\% \\
Reference Answer & 0.73 & 1.06 & +45\% \\
\bottomrule
\end{tabular}
\end{table}

Instruction tuning has differential effects across bias types (Table~\\ref{tab:study1}). Rubric order and score ID biases decrease substantially after instruction tuning ($-44\\%$ and $-58\\%$ respectively), indicating that instruction-tuned models parse scoring formats more robustly. However, reference answer bias increases by $+45\\%$, showing that instruction-tuned models are more susceptible to exemplar-based biasing. This differential pattern supports the Instruction-Induced Attention Redistribution (IIAR) hypothesis: instruction tuning improves format robustness while increasing sensitivity to content-based features.
